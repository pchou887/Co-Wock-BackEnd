...
export async function getCoupon(coupon_id: number) {
  const [results] = await pool.query(`SELECT * FROM coupons WHERE id = ?`, [coupon_id])
...
export async function checkCoupon(user_id: number, coupon_id: number) {
  const [results] = await pool.query(`SELECT * FROM user_coupons WHERE user_id = ?`, [user_id])
  const user_coupon = z.array(checkCouponSchema).parse(results)
  const result = user_coupon.filter((ele) => ele.coupon_id === coupon_id)
  console.log(user_coupon, result)
  const used = result[0] ? result[0].used : 1
  return used ? false : true
}

export async function UseCoupon(coupon_id: number) {
  const [results] = await pool.query(`UPDATE user_coupons SET used = true WHERE coupon_id = ?`, [coupon_id])
  return results
}
...
export async function createRecord(user_id: number) {
  ...
  const [results] = await pool.query(`INSERT INTO  week_straws (user_id, expire_time) VALUES(?,?)`, [
    user_id,
    expireTime,
  ])
  return results
...
export async function updateRecord(user_id: number, expireTime: string) {
  const [results] = await pool.query(`UPDATE week_straws set expire_time="${expireTime}" WHERE user_id=${expireTime}`)
  return results
}
